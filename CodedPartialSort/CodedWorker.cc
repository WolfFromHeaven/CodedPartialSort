  ...
  for ( auto init = inputSet.begin(); init != inputSet.end(); init++ ) {
    ...
    inputFile.seekg( 0, ios::beg );
		...
		for(unsigned int i = 0; i < numLine; i++){
		  ...
		  keyValueArray.push_back( buff );
			...
			fileData[inputId].push_back( buff );
		} 
    ...
    for ( unsigned int i = 0; i < conf->getNumReducer(); i++ ) {
      inputPartitionCollection[ inputId ][ i ] = new LineList;
		...
		nth_element(keyValueArray.begin(), keyValueArray.begin()+K, keyValueArray.end(), Sorter(conf->getKeySize()));
		vector<unsigned char*> top_K(keyValueArray.begin(), keyValueArray.begin()+K );	
		sort( top_K.begin(), top_K.end(), Sorter(conf->getKeySize()) );	
    ...
    // Build trie
		...
		for( auto it = inputSet.begin(); it != inputSet.end(); ++it ) {
			...
			for ( unsigned long j = 0; j < fileData[inputId].size(); j++ ) {
				unsigned int wid = trie->findPartition( fileData[inputId][j] );
				//cout<<wid<<" ";
				if( wid == conf->getNumReducer()-1 ) {
					unsigned char* buff = new unsigned char[ conf->getKeySize() + 1 ];
					memcpy( buff, fileData[inputId][j], conf->getKeySize() );
					if( !cmpKeyInverse ( buff, maximumKeyTopK, conf->getKeySize()) ){
							inputPartitionCollection[ inputId][ wid ]->push_back( fileData[inputId][j] );
					}
				}     
				else inputPartitionCollection[ inputId][ wid ]->push_back( fileData[inputId][j] );
		  }
		}
