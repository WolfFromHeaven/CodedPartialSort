...
/*  for ( unsigned int i = 1; i < conf->getNumReducer(); i++ ) {
  ...
  // EXECUTE MAP PHASE
  time = clock();
  //LineList topK = execMap();
	execMap();
  time = clock() - time;
  rTime = double( time ) / CLOCKS_PER_SEC;  
  MPI::COMM_WORLD.Gather( &rTime, 1, MPI::DOUBLE, NULL, 1, MPI::DOUBLE, 0 );
	//outputLocalListKeysTemp();      

  // EXECUTE ENCODING PHASE
  time = clock();
  //MaxElement maxEl = execEncoding(topK);
	execEncoding();
	//cout<<maxEl.size()<<"\n";
  time = clock() - time;
  rTime = double( time ) / CLOCKS_PER_SEC;
  ...
  //cout << rank << ": Shuffle phase takes " << double( time ) / CLOCKS_PER_SEC << " seconds.\n";
	...
	//LineList CompleteList[conf->getNumReducer()];
  ...
  // // cout << rank << ": Additional decoding phase takes " << double( time ) / CLOCKS_PER_SEC << " seconds.\n";    
	...
	sprintf( buff, "%s%s_%u", conf->getOutputPath(), "NoSort", rank - 1);
...
/*	cout<<rank<<" : ";
	...
	cout<<"\n";*/
	TopKCollection topKCollection;
	LineList keyValueArray;
	LineList fileData;
	//unordered_map<unsigned char*, unsigned int>FileId;
  ...
  for ( auto init = inputSet.begin(); init != inputSet.end(); init++ ) {
    ...
    //PartitionCollection& pc = inputPartitionCollection[ inputId ];//partition collection associated with files from inputIde
		...
		for(unsigned int i = 0; i < numLine; i++){
		  ...
		  keyValueArray.push_back( buff );
			fileData.push_back( buff );
			//FileId[buff] = inputId;
			//FileId.push_back(make_pair(buff, inputId));
		} 
    ...
    for ( unsigned int i = 0; i < conf->getNumReducer(); i++ ) {
      inputPartitionCollection[ inputId ][ i ] = new LineList;
		...
		sort( fileData.begin(), fileData.end(), Sorter(conf->getKeySize()) );

		//if(topKCollection[inputId].size() == 0){	
		vector<unsigned char*> temp(fileData.begin(), fileData.begin()+K );	
		topKCollection[inputId] = temp;
		//}

		//FileId[keyValueArray] = inputId;
		fileData.clear();
		inputFile.close();
	}
		...
		t3 = clock()-t3;
		double tt3;
		tt3 = double( t3 ) / CLOCKS_PER_SEC; 
		cout<<"Reading : "<<tt3<<endl;

/*	  char buff[ MAX_FILE_PATH ];
		sprintf( buff, "%s%s_%u", conf->getOutputPath(), "KeysTemp", rank - 1 );
		ofstream outputFile( buff, ios::out | ios::binary | ios::trunc );
		for ( auto it = keyValueArray.begin(); it != keyValueArray.end(); ++it ) {
			outputFile.write( ( char* ) *it, conf->getKeySize() );
			outputFile.write("\n",1);  
		}
		outputFile.close();*/
		...
		//partial_sort( keyValueArray.begin(), keyValueArray.begin()+K, keyValueArray.end(), reverseSorter(conf->getKeySize()) );
		sort( keyValueArray.begin(), keyValueArray.end(), Sorter(conf->getKeySize()) );

		vector<unsigned char*> top_K(keyValueArray.begin(), keyValueArray.begin()+K );
		/*for(auto it = keyValueArray.begin(); it != keyValueArray.begin()+K; ++it ){
				top_K.push_back(*it);
		}*/
		...
		LineList minMax;
		minMax.push_back((top_K.front())); //min
		minMax.push_back((top_K.back())); //max
		...
		for ( unsigned int i = 1; i < conf->getNumReducer(); i++ ) {
		  ...
		  partitionList.push_back( buff ); //cout<<i<<"\n";
		}
    ...
    // Crate lists of lines
		/*for ( unsigned long j = 0; j < K; j++ ) {
		  for ( unsigned int i = 0; i < conf->getNumReducer(); i++ ) {
		    inputPartitionCollection[ FileId[top_K[j]] ][ i ] = new LineList;
		  }
		}*/


    // Partition data of in the input file associated with inputID among the 3 partitions
    /*for ( unsigned long i = 0; i < K; i++ ) {
			unsigned int wid = trie->findPartition( top_K[i] );      
			inputPartitionCollection[ FileId[top_K[i]]][ wid ]->push_back( top_K[i] );
    }*/

		/*char buff[ MAX_FILE_PATH ];
		sprintf( buff, "%s%s_%u", conf->getOutputPath(), "topK", rank - 1);
		ofstream outputFile( buff, ios::out | ios::binary | ios::trunc );
		...
		/*for ( unsigned long i = 0; i < keyValueArray.size(); i++ ) {
			unsigned int wid = trie->findPartition( keyValueArray[i] );      
			inputPartitionCollection[ FileId[keyValueArray[i]]][ wid ]->push_back( keyValueArray[i] );
    }*/

		
		for( auto it = inputSet.begin(); it != inputSet.end(); ++it ) {
			unsigned int inputId = *it;
			for ( unsigned long j = 0; j < topKCollection[inputId].size(); j++ ) {
				unsigned int wid = trie->findPartition( topKCollection[inputId][j] );
				//cout<<wid<<" ";
				if( wid == conf->getNumReducer()-1 ) {
					unsigned char* buff = new unsigned char[ conf->getKeySize() + 1 ];
					memcpy( buff, topKCollection[inputId][j], conf->getKeySize() );
					if( !cmpKeyInverse ( buff, maximumKeyTopK, conf->getKeySize()) ){
							inputPartitionCollection[ inputId][ wid ]->push_back( topKCollection[inputId][j] );
					}
				}     
				else inputPartitionCollection[ inputId][ wid ]->push_back( topKCollection[inputId][j] );
		  }
		}
		...
		cout<<"Mapping : "<<tt<<endl;
		//return top_K;
}



//MaxElement CodedWorker::execEncoding(LineList LL)
void CodedWorker::execEncoding()
{
	//MaxElement maxElement;

  vector< NodeSet > subsetS = cg->getNodeSubsetSContain( rank );//all subsets of size r+1 containing node == rank
  unsigned lineSize = conf->getLineSize();
  for( auto nsit = subsetS.begin(); nsit != subsetS.end(); nsit++ ) {
    SubsetSId nsid = cg->getSubsetSId( *nsit );// returns integer value corresponding to subset *nsit
	...
	continue;
      ...
      LineList* ll = inputPartitionCollection[ fid ][ partitionId ];
			/*for(auto it = ll->begin(); it != ll->end(); ++it){
						if(find (LL.begin(), LL.end(),*it) == LL.end()) ll->erase(it);
			}*/
			
			//maxElement[vplist.back()] = ll->back();//*max_element(ll->begin(),ll->end(), reverseSorter(conf->getKeySize()));
			//cout<<ll->back()<<"\n\n\n";
	...
	for( unsigned long long j = 0; j < chunkSize; j++ ) {
	  memcpy( chunk + j * lineSize, *lit, lineSize );
		...
		/*if( ci == rankChunk && (j == 0 || j == chunkSize-1) ) {
			unsigned char* buff = new unsigned char[lineSize ];
			memcpy( buff, *lit, lineSize );
			maxElement[vplist.back()][rankChunk].push_back(buff);
			//cout<<buff<<"\n\n";
		}*/
	  lit++;
	}
	DataChunk dc;
	dc.data = chunk;
	dc.size = chunkSize; //cout<<ll->size()<<"\n";
	...
	memcpy( chunk + j * lineSize, *lit, lineSize );
		/*if(numPart == rankChunk+1 && (j == 0 || j == chunkSize-1) ) {
			unsigned char* buff = new unsigned char[lineSize ];
			memcpy( buff, *lit, lineSize );
			maxElement[vplist.back()][rankChunk].push_back(buff);
			//cout<<buff<<"\n\n";
...
//void CodedWorker::execShuffle(MaxElement maxE)
      ...
      MPI::Intracomm mcComm = multicastGroupMap[ nsid ];
		  ...
		  /*for( auto qit = nsit->begin(); qit != nsit->end(); qit++ ) {
		    ...
		    unsigned long fid = cg->getFileIDFromNodeSet( inputIdx );
				...
				if ( rank == activeId ) {
					int rootId = mcComm.Get_rank();
					mcComm.Bcast( (maxE[Vpair( destId, fid )][rankChunk]).front(), 2*conf->getLineSize(), MPI::UNSIGNED_CHAR, rootId );
				}
  ...
  for( auto init = inputSet.begin(); init != inputSet.end(); init++ ) {
    ...
    for( auto lit = ll->begin(); lit != ll->end(); lit++ ) {
      ...
      localList.push_back( buff );
			//CompleteList[inputId].push_back( buff );
    }
...
{  
  ...
  // }
  // stable_sort( localList.begin(), localList.end(), reverseSorter( conf->getKeySize() ) );
