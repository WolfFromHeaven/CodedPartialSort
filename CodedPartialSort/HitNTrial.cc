...
void CodedWorker::execMap()
{
  ...
  InputSet inputSet = cg->getM( rank );
	...
	//map<vector<unsigned char*>, unsigned int>FileId;

	map<unsigned char*, unsigned int, KeyOrder>FileId;
	//pair<unsigned char*, unsigned int> foo;
	//vector<pair<unsigned char*, unsigned int>> FileId;
  ...
  for ( auto init = inputSet.begin(); init != inputSet.end(); init++ ) {
    ...
    //PartitionCollection& pc = inputPartitionCollection[ inputId ];//partition collection associated with files from inputIde
		...
		cout<<rank<<" : Creating Map = "<<tt3<<endl;
		...
		//sort( FileId.begin(), FileId.end(), KeyOrder );		
		//sort( keyValueArray.begin(), keyValueArray.end(), Sorter(conf->getKeySize()) );	
		//sort( FileId.begin(), FileId.end(), Sorter(conf->getKeySize()) );	


		//vector<unsigned char*> top_K(keyValueArray.begin(), keyValueArray.begin()+K );
		//vector<pair<unsigned char*, unsigned int>> top_K(FileId.begin(), FileId.begin()+K );
		
		
		LineList top_K;
		for(auto it = FileId.begin(); it != next(FileId.begin(),K); ++it){
			top_K.push_back(it->first);
		}

		//vector<unsigned char*> top_K( FileId.begin(), FileId.begin()+K );

		t2 = clock()-t2;
		double tt2;
		tt2 = double( t2 ) / CLOCKS_PER_SEC; 
		cout<<rank<<" : Extracting K values = "<<tt2<<endl;

		//SEND THE TOPK VALUES FROM EACH SERVER TO THE MASTER NODE
		LineList minMax;
		minMax.push_back((top_K.front())); //.first);
		minMax.push_back((top_K.back())); //.first);
   ...
   /* for ( unsigned long i = 0; i < K; i++ ) {
		...
		for ( unsigned long j = 0; j < K; j++ ) {
		  for ( unsigned int i = 0; i < conf->getNumReducer(); i++ ) {
		    inputPartitionCollection[ FileId[top_K[j]] ][ i ] = new LineList;
				//inputPartitionCollection[ top_K[j].second ][ i ] = new LineList;
		  }
		}
    ...
    for ( unsigned long i = 0; i < K; i++ ) {
      //unsigned int wid = trie->findPartition( top_K[i].first );
			unsigned int wid = trie->findPartition( top_K[i] );
      // pc[ wid ]->push_back( buff );  
			//cout<<wid<<"\n";    
			...
			//inputPartitionCollection[ top_K[i].second ][ wid ]->push_back( top_K[i].first );
    }

		t1 = clock()-t1;
		double tt;
		tt = double( t1 ) / CLOCKS_PER_SEC; 
		cout<<rank<<" : Partitioning = "<<tt<<endl;
...
void CodedWorker::outputLocalListKeys()
{
  char buff[ MAX_FILE_PATH ];
...
LineList extract_keys(map<unsigned char*, unsigned int, KeyOrder> const& input_map) {
  LineList retval;
